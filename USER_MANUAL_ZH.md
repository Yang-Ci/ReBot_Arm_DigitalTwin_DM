...
重力补偿启动时会默认进入“低阻助力”。该模式不会自行起步：只有检测到夹爪已经被手推动后，才沿运动方向叠加小幅力矩；速度刚超过阈值时先给 30% 起手助力，便于从闭合状态打开。静止、接近开合限位或速度超过保护阈值时助力为零。启动前必须取下被夹物体、让手指远离夹点，并准备点击“释放夹爪”立即退出。任何电动开合命令都会自动退出低阻助力并恢复位置控制。

默认助力参数位于 `reBotArm_ros2_DM/src/rebotarm_bringup/config/driver_params.yaml`。`gripper_assist_torque` 默认为电机侧 `0.04N·m`，`gripper_assist_kd` 为 `0.001`，`gripper_assist_breakaway_fraction` 为 `0.30`，用于降低手掰阻尼并改善静止起手；控制器硬限制仍不超过 `0.08N·m`。当前助力已达到建议首次调整上限，继续调软前应先在真机上确认没有异常发力。
